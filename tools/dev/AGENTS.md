...
- **Non-interactive shells bypass direnv.** `.envrc` rewrites `DATABASE_URL` to the worktree-scoped database, but agent shells and non-interactive sessions do not execute `.envrc`. `pnpm db:migrate` resolves the checkout through `applyDevEnvToProcess`; other direct test and admin commands must do the same or source `print-worktree-env.ts` output before touching a database.
