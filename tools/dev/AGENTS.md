...
- **Worktree cleanup is commit-authorized.** `pnpm dev:prune-worktrees` binds eligibility to the planned branch OID and revalidates it before every action. Exact PR evidence includes base, head OID, and repository owner. Keep authorization policy in `lib/worktree-cleanup-eligibility.ts`, orchestration in `prune-worktrees.ts`, and actions in `lib/worktree-cleanup.ts`; never authorize cleanup from a branch name alone.
