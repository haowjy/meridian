...
- **Target readiness.** `--target` refuses dirty or locked worktrees while planning and revalidates both gates before each worktree-bound teardown action, so database cleanup never precedes either Git refusal.
- **Auto readiness is separate.** `--auto` accepts exact merged-PR evidence only—ancestry alone is not stale evidence—and skips dirty or locked worktrees, active Meridian work items, live tmux dev sessions, and processes whose cwd is inside the worktree. These gates run during planning and again before each worktree-bound teardown action.
