 ...
 * nothing, dismisses itself, and never claims to know where the passage went.
 */
...
export function PassageNotice({ documentId }: { documentId: string | null }) {
  const raisedAt = usePassageNotice(documentId);

  useEffect(() => {
    if (raisedAt === null) return;
    const timer = setTimeout(dismissPassageNotice, DISMISS_AFTER_MS);
