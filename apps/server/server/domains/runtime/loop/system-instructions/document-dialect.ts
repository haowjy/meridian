...
export const DOCUMENT_DIALECT_CORE_INSTRUCTION = [
  ...
  `- Link to another document as \`${DOCUMENT_DIALECT_CONTRACT.wikilink.wire}\`. Use \`${DOCUMENT_DIALECT_CONTRACT.wikilink.labeledWire}\` for separate display text. The text after the pipe is literal prose; only the destination before it is resolved. A scoped Context URI may be the destination.`,
