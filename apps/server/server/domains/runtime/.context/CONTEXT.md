...
| `work-context.ts` / `system-update-delivery.ts` | Renders the current Work plus the 20 most recently active sibling Works. Work-list changes queue one tagged user-role transcript turn per live project thread; running turns flush it at their completion boundary without touching the frozen prompt. `threadChanged` is the single-thread seam used by future rebind commands. |
