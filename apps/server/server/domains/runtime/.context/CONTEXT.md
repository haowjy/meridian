...
| Model-request inspection | Immediately before `Gateway.stream()`, the orchestrator hands the complete provider-neutral `GenerateRequest` to the composed capture port without `signal` or diagnostic correlation. The no-op adapter discards it without serialization. Local dev/test capture preallocates the `gatewayCallId` shared with lifecycle events, serializes once, and retains at most 200 records, 2 MiB per request, and 16 MiB total in memory; oversized bodies retain their digest and metadata without being parsed into a second object. Exact-call queries return the match plus its adjacent prefix context. The environment gate cannot be enabled in staging or production. Content never enters `EventSink`, thread snapshots, the event journal, or JSONL. |
