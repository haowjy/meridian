...
| Model-request inspection | Immediately before `Gateway.stream()`, the orchestrator records the complete provider-neutral `GenerateRequest` without `signal` or diagnostic correlation. It preallocates the `gatewayCallId` shared with lifecycle events. Dev composition retains at most 200 records, 2 MiB per request, and 16 MiB total in memory; oversized bodies retain their digest and metadata. The owner-gated thread route returns this content and exact loss counters. Content never enters `EventSink`, thread snapshots, the event journal, or JSONL. |
