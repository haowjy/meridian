...
- `database`, `contracts`, `apps/server`, `apps/app`: add the authoritative
  Context metadata catalog with atomic metadata publication, persisted tracked
  and binary classification, whole-commit replay, Work/no-Work authority,
  ...
  normalized stable-ID cache shared by direct-child tree rows, picker, opening,
  and placement consumers; remove the recursive tree transport and duplicate
  client owner.
...
- `apps/server`, `packages/database`: Work and conversation-binding mutations
  ...
  cross-server run ownership keeps sweeps out of live remote turns without
  admitting same-process reentry, competing claims hydrate the committed
  update, and completion cleanup remains recoverable when delivery fails.
...
- `agent-edit`: find-all now replaces exact same-block ranges without re-authoring
  ...
  including whole-block structural carries that replace their input block (#380).
