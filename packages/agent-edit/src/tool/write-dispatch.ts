...
import { formatTurnDiff, status } from "./response-format.js";
    ...
    switch (command.command) {
      ...
      case "diff": {
        if (!context.threadId || !context.turnId) {
          return status("invalid_write", "Diff requires the current thread and turn context.");
        }
        if (!input.turnDiffQuery) {
          return status("invalid_write", "Turn diff queries are not available in this host.");
        }
