...
- Preserve structural compatibility between server document logic and the app's
  TipTap editor. **Nothing enforces this** — the parity test was deleted. Any
  attr or node added here has to be mirrored by hand in
  `apps/app/src/core/editor/extensions/meridian-extensions.ts`, and a schema the
  two sides disagree on decodes on one and fails on the other.
